...
def get_database_trips(supabase: Client, date: str) -> List[Dict]:
    ...
    try:
        ...
        # HISTORICAL FIX: landing_id now stored per trip for boats that move between landings
    ...
    if output_file:
        output_data = {
            ...
            'summary': {
                ...
                'failed': failed,
                'errors': errors,
                'pass_rate': round(passed / total_dates * 100, 2) if total_dates > 0 else 0
            },
            'reports': reports,
            'timestamp': datetime.utcnow().isoformat()
        }
