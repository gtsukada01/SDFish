...
### Build
- `assets/main.js` - Rebuilt (2.0MB)
- `assets/styles.css` - Rebuilt (1.6KB)
...
```bash
# Dry run test
python3 boats_scraper.py --start-date 2025-10-14 --dry-run
...
1. **Weekly Scraping**: Run scraper weekly to keep database current
   ```bash
   python3 boats_scraper.py --start-date 2025-10-16 --end-date $(date +%Y-%m-%d)
   ```
